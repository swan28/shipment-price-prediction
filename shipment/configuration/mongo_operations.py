...
class MongoDBOperation:
    def __init__(self):
        self.DB_URL = DB_URL
        self.client = MongoClient(self.DB_URL, tls=True, tlsAllowInvalidCertificates=True) # tls->Use TLS connections, tlsAllowInvalidCertificates->Specifies whether or not the driver should error when the server’s TLS certificate is invalid
        # print("mongooperations folder - init ---> mongourl", self.DB_URL)
    ...
    def get_collection_as_dataframe(self, db_name, collection_name) -> DataFrame:
        ...
        try:
            ...
            # Dropping the _id column from the DF
